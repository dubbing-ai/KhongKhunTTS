...
def process_audio(audio_file_path, api_key) -> Tuple[str, str]:
    """
...
4.  **Repetitive Words:** Ensure that repetitive words or phrases in the audio (represented in Thai by 'ๆ') are translated with the full repetition. For example, 'เสื้อสีดำๆ' should be translated as 'เสื้อสีดำดำ', and 'ไฟไหม้ๆ' should be translated as 'ไฟไหม้ ไฟไหม้'.
5.  **English to Thai Sound-Alike:** If there is no direct Thai translation for an English word (like a name), use a Thai word that sounds similar. For example, 'Edward' -> 'เอ็ดเวิร์ด' or 'Maple' -> 'เมเปิ้ล'.
...
**Original Transcription:** I have twenty-four black computers numbered one, two, three, which are with Edward, who has twelve computers, and he is the one shouting 'burning! burning!'

**Thai Translation:** ฉันมีคอมพิวเตอร์หมายเลขหนึ่งสองสามสีดำดำอยู่ยี่สิบสี่เครื่อง ซึ่งอยู่กับเอ็ดเวิร์ดสิบสองเครื่อง และเขาคือคนที่กำลังกรีดร้องว่า ไฟไหม้ ไฟไหม้

Please process the provided audio file and follow these instructions meticulously.""",
                    },
        ...
        if 'candidates' in response_data and len(response_data['candidates']) > 0:
            ...
            full_text = "".join([part['text'] for part in content['parts'] if 'text' in part])
            ...
            for section in sections:
                if section.startswith("Original Transcription:"):
                    transcription = section.replace("Original Transcription:", "").strip()
                elif section.startswith("Thai Translation:"):
                    translation = section.replace("Thai Translation:", "").strip()
...
def main():
    ...
    if file_size_mb > 10:
        print(f"Warning: The file size is {file_size_mb:.2f} MB which may exceed API limits.")
